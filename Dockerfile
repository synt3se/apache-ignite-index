...
COPY --from=build /src/vector-index-node/target/vector-index-node-*.jar /opt/ignite/apache-ignite/libs/ext/
...
COPY --from=build /src/index-vector-server/target/vector-server-*.jar /app/app.jar
...
COPY --from=build /src/telegram-backend/target/telegram-bot-*.jar /app/app.jar
ENTRYPOINT ["java","-jar","/app/app.jar"]
